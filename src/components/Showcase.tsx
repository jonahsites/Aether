...
const sections = [
  {
    label: "01 — ASCENT",
    title: "The Vision of\nUrus SE",
    desc: "A Super SUV born for the mountain peak. 800 CV of calculated force.",
    tags: ["HYBRID", "800 CV", "ALPINE"],
    color: "#05070A",
    accent: "#9AC8FF"
  },
  {
    label: "02 — GLACIER",
    title: "The Pulse of\nRevuelto",
    desc: "V12 Hybrid power that echoes through the valley floor. Pure technical poetry.",
    tags: ["V12", "1015 CV", "ETHEREAL"],
    color: "#05070A",
    accent: "#E2E8F0"
  },
  {
    label: "03 — ZENITH",
    title: "The Silence of\nMansory Wraith",
    desc: "Absolute luxury, whispered in carbon and starlight. The pinnacle of Alpine comfort.",
    tags: ["V12", "BESPOKE", "ZENITH"],
    color: "#05070A",
    accent: "#9AC8FF"
  },
  {
    label: "04 — SUMMIT",
    title: "The Soul of\nCorvette Z06",
    desc: "Flat-plane V8 precision for the sharpest mountain curves. Adrenaline engineered.",
    tags: ["TRACK", "670 HP", "VELOCITY"],
    color: "#05070A",
    accent: "#FFFFFF"
  }
];
  ...
  return (
    <div className="relative z-10 pointer-events-none" ref={containerRef}>
      ...
      {sections.map((section, i) => (
          <section
            key={i}
            className={`relative w-full h-screen flex flex-col items-center justify-center p-10 md:p-24 transition-colors duration-1000`}
            style={{ backgroundColor: `${section.color}` }}
          >
            {/* Background atmosphere */}
            <div className="absolute inset-0 atmosphere-bg opacity-30 pointer-events-none" />
            
            {/* Floating Index */}
            <div className="absolute top-24 left-1/2 -translate-x-1/2 font-serif italic text-white/5 text-[30vw] select-none pointer-events-none">
              {i + 1}
            </div>

            <div className={`relative z-10 max-w-4xl w-full flex flex-col items-center text-center pointer-events-auto`}>
              <motion.div 
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                className="text-[10px] font-light uppercase tracking-[0.6em] mb-12 text-aether-ice"
              >
                {section.label}
              </motion.div>
              
              <h2 className="text-6xl md:text-[8vw] font-serif italic mb-16 leading-[0.9] tracking-tighter text-white">
                {section.title}
              </h2>

              <div className="flex flex-col items-center gap-12">
                <p className="text-white/40 font-serif italic text-xl md:text-2xl leading-relaxed max-w-2xl">
                  {section.desc}
                </p>
                ...
                <div className="flex flex-wrap justify-center gap-6">
                  {section.tags.map((tag, j) => (
                    <span 
                      key={j} 
                      className="px-8 py-3 glass-morphism rounded-full font-sans text-[9px] uppercase tracking-[0.3em] font-light text-white transition-all hover:bg-white hover:text-black"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              {/* Decorative line */}
              <div className="mt-20 w-px h-24 bg-linear-to-b from-aether-ice/40 to-transparent" />
            </div>
          </section>
      ))}
    </div>
